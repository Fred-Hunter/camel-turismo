...
export class RaceDrawing {
    ...
    public drawRaceCourse(raceParams: RaceDrawingParameters, scale: number) {
    ...
    private getObject(): Array<{ x: number, y: number, colour: Colour }> {
    ...
    private drawCamel(camel: RacingCamel, race: Race) {
        ...
        let camelCoords: Array<{ x: number, y: number, colour: string }> = [];
    ...
    private drawChevrons(camel: RacingCamel, xCoord: number, yCoord: number) {
        const ctx = this._camelCanvas.getContext("2d")!;
        ctx.save();
        ctx.lineWidth = 2;
        const chevronSize = 4;
